...
public class purchase_ctr implements Initializable {
    ...
    @FXML
    public JFXButton purchase_details_btn;
    ...
    @FXML
    private JFXButton purchase_add_back;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        try {
            ...
            ResultSet count = con.createStatement().executeQuery("Select count(*)+1 from tbl_purchase_details");
            if (count.next()) {
                String sum = "PU";
                sum = sum + count.getString("count(*)+1");
                purchase_id.setText(sum);
    ...
    public void purchase_details_btn(ActionEvent event) {

        try {
        Connection con= DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/inventory_db","root","");
        Statement stmt=con.createStatement();
        p4=purchase_id.getText();
        p1=product_id.getText();
        p2=supplier_id.getText();
       ...
       // LocalDate value = date.getValue();
        //System.out.println(value);
            ...
            stmt.executeUpdate("insert into tbl_purchase_details values('"+p4+"','"+p1+"','"+p2+"','"+p3+"','"+date12+"')");
    ...
    public void purchase_add_back(ActionEvent event) {

        Parent parent = null;
        try {
            parent = FXMLLoader.load(this.getClass().getResource("purchasetable.fxml"));
        } catch (IOException e) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Oops Something went wrong!!!");
